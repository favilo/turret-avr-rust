...
use crate::Serial;
use arduino_sys::{__get_servo, __get_servoCount, __inc_servoCount, digitalWrite};
...
impl<State> Servo<State> {
    ...
    pub fn new<PIN: PinOps<Dynamic = Dynamic>>(
        ...
        if unsafe { __get_servoCount() } >= MAX_SERVOS as u8 {
            return Err(ServoError::TooManyServos);
        }
        let index = unsafe { __inc_servoCount() };
        let servo = unsafe { __get_servo(index) };
        unsafe { (*servo).ticks = us_to_ticks(DEFAULT_PULSE_WIDTH) };
        Ok(Servo {
    ...
    fn is_timer_active() -> bool {
        for channel in 0..MAX_SERVOS as u8 {
            let servo = unsafe { __get_servo(channel) };
            if unsafe { (*servo).Pin.isActive() != 0 } {
                return true;
            }
        }
        return false;
    ...
    fn is_attached(&self) -> bool {
        unsafe { (*__get_servo(self.index as u8)).Pin.isActive() != 0 }
    ...
    #[allow(dead_code)]
    fn read_us(&self) -> u16 {
        let servo = unsafe { __get_servo(self.index as u8) };
        let ticks = unsafe { *servo }.ticks;
        // let ticks = avr_device::interrupt::free(|_| unsafe { SERVOS[self.index].ticks.get() });
        ticks_to_us(ticks) + TRIM_DURATION as u16
    }
}
...
impl Servo<ServoDetached> {
    ...
    pub fn attach_with_limits(self, min: i16, max: i16) -> Servo<ServoAttached> {
        ...
        // });
        let servo = unsafe { __get_servo(self.index as u8) };
        unsafe { (*servo).Pin.set_isActive(1) };
        Servo {
            index: self.index,
            min,
            max,
...
impl Servo<ServoAttached> {
    ...
    pub fn detach(self) -> Servo<ServoDetached> {
        ...
        // });
        let servo = unsafe { __get_servo(self.index as u8) };
        unsafe { (*servo).Pin.set_isActive(0) };
    ...
    pub fn write_us(&self, value: i16, serial: &mut Serial) {
        ...
        // });
        let servo = unsafe { __get_servo(self.index as u8) };
        unsafe { (*servo).ticks = value as u16 };
    }
}
...
fn TIMER1_COMPA() {
    use arduino_sys::__get_channel;

    let tc1 = unsafe { TC1.as_ref().unwrap() };
    // if( Channel[timer] < 0 )
    // if CHANNEL.load(Ordering::SeqCst) < 0 {
    if unsafe { *__get_channel(0) } < 0 {
        //   *TCNTn = 0; // channel set to -1 indicated that refresh interval completed so reset the timer
        tc1.tcnt1.write(|w| w.bits(0));
    } else {
        //   if( SERVO_INDEX(timer,Channel[timer]) < ServoCount && SERVO(timer,Channel[timer]).Pin.isActive == true )
        if unsafe { *__get_channel(0) } < unsafe { __get_servoCount() as i8 } {
            if unsafe { (*__get_servo(*__get_channel(0) as u8)).Pin.isActive() != 0 } {
                // if let Some(servo) = unsafe { SERVOS.get_mut(CHANNEL.load(Ordering::SeqCst) as usize) } {
                // if servo.attached {
                //     digitalWrite( SERVO(timer,Channel[timer]).Pin.nbr,LOW); // pulse this channel low if activated
                // servo.pin.set_low();
                unsafe { digitalWrite((*__get_servo(*__get_channel(0) as u8)).Pin.nbr(), 0x0) };
            }
        }
    }
    // Channel[timer]++;    // increment to the next channel
    // CHANNEL.store(CHANNEL.load(Ordering::SeqCst), Ordering::SeqCst);
    unsafe { *__get_channel(0) += 1 };

    // if( SERVO_INDEX(timer,Channel[timer]) < ServoCount && Channel[timer] < SERVOS_PER_TIMER) {
    // if let Some(servo) = unsafe { SERVOS.get_mut(CHANNEL.load(Ordering::SeqCst) as usize) } {
    if unsafe { *__get_channel(0) } < unsafe { __get_servoCount() as i8 }
        && unsafe { *__get_channel(0) < MAX_SERVOS as i8 }
    {
        //   *OCRnA = *TCNTn + SERVO(timer,Channel[timer]).ticks;
        let servo = unsafe { __get_servo(*__get_channel(0) as u8) };
        tc1.ocr1a
            .write(|w| w.bits(tc1.tcnt1.read().bits() + unsafe { *servo }.ticks));
        //   if(SERVO(timer,Channel[timer]).Pin.isActive == true)     // check if activated
        if unsafe { *servo }.Pin.isActive() != 0 {
            //     digitalWrite( SERVO(timer,Channel[timer]).Pin.nbr,HIGH); // its an active channel so pulse it high
            // servo.pin.set_high();
            unsafe { digitalWrite((*servo).Pin.nbr(), 0x1) };
        }
    } else {
...
fn ticks_to_us<N: PrimInt>(ticks: N) -> N {
    (ticks * N::from(8).unwrap()) / N::from(CLOCK_CYCLES_PER_MICROSECOND).unwrap()
}
